 ...
 * license : MIT
 */
CommentryPlayer=new Commentry;const searchMixin={data:function(){return{query:"",users:[],isGlobalSearch:!0,isLoading:!1,error:null,tag:"",userGroupId:0,showBettors:!0}},computed:{topResult:function(){return this.users.length>0?this.users[this.users.length-1]:null},userPath:function(){return this.users.length>0?this.usernames.join(" / "):null},usernames:function(){return this.users.map(function(n){return n.username})},apiUrl:function(){return this.isGlobalSearch?"/api/users/search?query="+this.query:"/api/v2/users?q="+this.query+"&maxResults=1"}},created:function(){window.searchConfig!==undefined&&(this.isGlobalSearch=window.searchConfig.isGlobalSearch,this.showBettors=window.searchConfig.showBettors)},methods:{search:function(){if(this.query!==""){this.isLoading=!0;this.error=null;const n=this;$.get(this.apiUrl,function(t){t==null||t.length==0?(n.users=[],n.error="User not found"):n.showBettors||t[t.length-1].type!=4?n.users=t:(n.users=[],n.error="Dealer not found")}).fail(function(){n.users=[];n.error="User not found"}).always(function(){n.isLoading=!1})}},POPUPWINDOW:function(n,t){const i=n+t;let r=window.open(i,"","height=800,width=650,titlebar=0,menubar=0");return window.focus&&r.focus(),!1}}},gridMixin={data:function(){return{gridItems:[],searchQuery:""}},created:function(){this.fetchUsers()},watch:{},computed:{filteredItems:function(){return this.searchQuery.length===0?this.gridItems:this.gridItems.filter(function(n){return n.tag!=null&&n.tag.toLowerCase().indexOf(this.searchQuery.toLowerCase())!==-1},this)},selectedItems:function(){return this.gridItems.filter(function(n){return n.selected})}},methods:{select:function(){const n=this;let t={id:this.topResult.id,username:this.topResult.username,path:this.userPath,tag:this.tag,UserGroupId:this.userGroupId};$.ajax({type:"POST",url:"/api/v2/userRefs",data:JSON.stringify(t),contentType:"application/json; charset=utf-8",dataType:"json",success:function(n){console.log(n)}}).always(function(){n.fetchUsers()})},fetchUsers:function(){const n=this;$.ajax({type:"GET",url:"/api/v2/userRefs",contentType:"application/json; charset=utf-8",dataType:"json",success:function(t){n.gridItems=t}})},remove:function(n){const t=this;$.ajax({type:"DELETE",url:"/api/v2/userRefs/"+n.id,contentType:"application/json; charset=utf-8",dataType:"json"}).always(function(){t.fetchUsers()})},toggleAll:function(){this.filteredItems.forEach(function(n){n.selected=!n.selected});this.gridItems.push({});this.gridItems.pop()}}};
/*! E3 General 
 * 2019 BetPro
 */
let svc_prefix="/api/",cid="";const LOGOUT_KEY="logout-event",LOGGED_OUT_URL="/Common/LoggedOut";if(window.addEventListener("storage",function(n){n.key==LOGOUT_KEY&&(document.location.href=document.location.origin+LOGGED_OUT_URL)}),$(function(){function n(n,i,r,u){$.ajax({type:"Get",url:u+"&Uid="+n+"&from="+i+"&to="+r,success:function(n){return document.getElementById("loadinggif").style.display="none",document.getElementById("sportreportdivdr").innerHTML=n,$(".Report-EventTypeId-new").click(function(n){n.preventDefault();document.getElementById("loadinggif").style.display="block";var u=$(this).data("id"),f=$(this).data("url");return t(cid,u,i,r,f),!1}),n.value===null||n.value===undefined||n.value.length===0?(document.getElementById("loadinggif").style.display="none",!0):void 0},error:function(){document.getElementById("loadinggif").style.display="none"}})}function t(n,t,i,r,u){$.ajax({type:"Get",url:u+"&Uid="+n+" &etid="+t+"&from="+i+"&to="+r,success:function(n){document.getElementById("loadinggif").style.display="none";document.getElementById("marketreportsdivdr").style.display="block";document.getElementById("marketreportsdivdr").innerHTML=n;convertAllToClientTime()},error:function(){document.getElementById("loadinggif").style.display="none"}})}$("#btn-logout").click(function(n){n.preventDefault();const t=$(this).attr("href");localStorage.setItem(LOGOUT_KEY,"logout"+Math.random());document.location.href=document.location.origin+t});$(".Report-EventTypeId-new-single").click(function(n){n.preventDefault();var t=$(this).data("id");return $("#ClientId").val(t),$("#ReportFilterForm").find("button").click(),!0});$(".Report-EventTypeId-new-frontPL").click(function(n){n.preventDefault();var t=$(this).data("id");return updateDates(),$("#EventTypeId").val(t),$("#ReportFilterForm").submit(),!0});$(".Commision-eventtypeid-For3rd").click(function(n){n.preventDefault();var t=$(this).data("id");return $("#EventTypeId").val(t),$("#ReportFilterForm").find("button").click(),!0});$(".Report-Client-new").click(function(t){t.preventDefault();var i=$(this).data("id"),r=$(this).data("from"),u=$(this).data("to"),f=$(this).data("url");return document.getElementById("marketreportsdivdr").style.display="none",document.getElementById("loadinggif").style.display="block",cid=i,n(i,r,u,f),!0});$(".Report-Client").click(function(n){n.preventDefault();var t=$(this).data("id"),i=localDateToUtc($("#From").val()),r=localDateToUtc($("#To").val()),u=$(this).data("url");return document.getElementById("marketreportsdiv").style.display="none",document.getElementById("loadinggif").style.display="block",load(t,i,r,u),!0});$(".tbl-datatable").length>0&&$(".tbl-datatable").DataTable({order:[[0,"asc"]]});$(".dt-desc").length>0&&$(".dt-desc").DataTable({order:[[0,"desc"]]});$("#news-ticker-foot").length>0&&$("#news-ticker-foot").webTicker({height:"25px"});$("#searchUsername").keypress(function(){})}),d2etid="",$("#searchUsers").length>0){searchVM=new Vue({el:"#searchUsers",mixins:[searchMixin],data:{},computed:{},created:function(){},methods:{}});countries=[];function n(){var t=document.getElementById("myInput").value,i=t.length,n;i>=2&&($.ajax({type:"Get",url:svc_prefix+"Users/",success:function(n){self.users=n;var t=self.users.length-1;countries.push(self.users[t].username)},error:function(){}}),n=[],$.each(countries,function(t,i){$.inArray(i,n)===-1&&n.push(i)}),autocomplete(document.getElementById("myInput"),n))}}SetupHeaders();typeof Vue!="undefined"&&(Vue.filter("format",function(n){return n===0||n===null||n===undefined?"":n===""?null:numeral(n).format("0,0")}),Vue.filter("formatForiegn",function(n){return n===0||n===""||n===null||n===undefined?"":n<1e3?numeral(n).format("0"):numeral(n).format("0.0a").toLocaleUpperCase()}),Vue.filter("formatForiegn2",function(n){return n===0||n===""||n===null||n===undefined?"":n<1e3?numeral(n).format("0"):numeral(n).format("0a").toLocaleUpperCase()}));$(".burger-toggle").on("click",function(){$("body").toggleClass("menu-collapsed");$(this).toggleClass("active")});isIOS=iOS();$(document).ready(function(){var n=$("#container").width();$(".box").width(n/5);n=$("body").width();n<1026&&($("body").addClass("menu-collapsed"),$(".burger-toggle").addClass("active"));$(window).resize(function(){var n=$("body").width();n<1026?($("body").addClass("menu-collapsed"),$(".burger-toggle").addClass("active")):n>1026&&($("body").removeClass("menu-collapsed"),$(".burger-toggle").removeClass("active"));FixVideoSize()})});let streams=[];streams.push({container:"#VBox1",el:"#streamVideo1",unreal:"#UnrealPlayer1",player:null});channelname="";class UWS{constructor(n,t,i){this.url=n;this.token=t;this.sessionToken=i;this.connection=null;this.balance=0;this.exposure=0;this.announcement=localStorage.getItem("announcement")||"";this.message=localStorage.getItem("dm")||""}connect(){this.connection=(new signalR.HubConnectionBuilder).withUrl(this.url,{accessTokenFactory:()=>this.token}).withAutomaticReconnect({nextRetryDelayInMilliseconds:n=>{return n.elapsedMilliseconds<36e5?n.previousRetryCount===0?0:n.previousRetryCount===1?2e3:n.previousRetryCount<10?5e3:1e4:null}}).configureLogging(signalR.LogLevel.Error).build();this.subscribeToEvents();this.connection.start().then(()=>{}).catch(n=>console.error("SignalR connection error: ",n))}subscribeToEvents(){this.connection.on("ReceiveFunds",n=>{this.balance=parseInt(n),this.updateBalance()});this.connection.on("ReceiveExposure",n=>{this.exposure=parseInt(n),this.updateBalance()});this.connection.on("ReceiveAnnouncement",n=>{this.announcement=n,localStorage.setItem("announcement",n),this.updateNews()});this.connection.on("ReceiveDM",n=>{this.message=n,localStorage.setItem("dm",n),this.updateNews()});this.connection.on("ReceiveBetSizes",n=>{localStorage.setItem("cricket",n.cricket),localStorage.setItem("bookmaker",n.bookMaker),localStorage.setItem("fancy",n.fancy),localStorage.setItem("casino",n.casino),localStorage.setItem("greyhound",n.greyhound),localStorage.setItem("race",n.race),localStorage.setItem("soccer",n.soccer),localStorage.setItem("tennis",n.tennis)});this.connection.on("SessionDeleted",n=>{this.sessionToken==n})}updateBalance(){let r=this.exposure<0?this.balance+this.exposure:this.balance;const n=document.querySelector(".wallet-balance"),i=document.querySelector(".wallet-exposure"),t=localStorage.getItem("symbol");n!=null&&(n.innerText="B: ",t!="null"&&t!=null&&(n.innerText+=t+" "),n.innerText+=(new Intl.NumberFormat).format(r));i!=="null"&&(i.innerText=" | L: "+(new Intl.NumberFormat).format(this.exposure))}keepAlive(){this.connection.invoke("KSA",this.sessionToken).catch(n=>console.error("KeepAlive error: ",n));this.sessionToken!==null&&this.sessionToken!==undefined&&setInterval(()=>{this.connection.invoke("KSA",this.sessionToken).catch(n=>console.error("KeepAlive error: ",n))},4.5*6e4)}updateNews(){let n=this.announcement+" - "+this.message;$("#news-ticker-foot").length>0&&$("#news-ticker-foot").webTicker("update",'<li data-update="item1"><b>'+n+"<\/b><\/li>","swap",!0,!1)}}