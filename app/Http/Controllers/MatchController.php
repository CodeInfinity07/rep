...
class MatchController extends Controller
{
    ...
    public function show($marketId)
    {
        ...
        if (!$apiKey) {
            return view($errorView)->with(['error' => 'API key not configured', 'balance' => $user->balance ?? 0, 'liable' => 0, 'active_bets' => 0, 'username' => $user->username ?? '']);
        }
        ...
        try {
            ...
            if (!$marketDetails) {
                return view($errorView)->with(['error' => 'Failed to fetch market details', 'balance' => $user->balance ?? 0, 'liable' => 0, 'active_bets' => 0, 'username' => $user->username ?? '']);
            }
            ...
            if (!$eventId) {
                return view($errorView)->with(['error' => 'Event ID not found', 'balance' => $user->balance ?? 0, 'liable' => 0, 'active_bets' => 0, 'username' => $user->username ?? '']);
            }
            ...
            return view($errorView)->with(['error' => 'Error: ' . $e->getMessage(), 'balance' => $user->balance ?? 0, 'liable' => 0, 'active_bets' => 0, 'username' => $user->username ?? '']);
        }
    }
    ...
    public function showCricketIdMatch($gmid, Request $request)
    {
        ...
        try {
            ...
            return view($errorView)->with(['error' => 'Error loading match: ' . $e->getMessage(), 'balance' => $user->balance ?? 0, 'liable' => 0, 'active_bets' => 0, 'username' => $user->username ?? '']);
        }
    }
