    ...
    <script>
        ...
        function loadMatches() {
            $.get('/api/cricket-matches', function(data) {
                const matches = data.soccer || [];
                const container = $('#matches-container');
                $('#loader').hide();
                
                if (matches.length === 0) {
                    container.html('<p>No soccer matches available at the moment.</p>');
                    return;
                }
                
                container.html(matches.map(match => {
                    const runners = match.runners || [];
                    const r1 = runners[0] || {};  // Home
                    const r2 = runners[1] || {};  // Draw
                    const r3 = runners[2] || {};  // Away
