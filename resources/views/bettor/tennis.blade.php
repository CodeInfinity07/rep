    ...
    <div class="row">
        <div class="col-12">
            <div class="alert alert-success mb-3">
                <strong>✓ BETTOR TENNIS PAGE</strong> - You are viewing the bettor interface
            </div>
            <h4 class="text-white mb-3">Tennis</h4>
            
            <div id="tennis-matches-container">
                <!-- Tennis matches will be populated here -->
            </div>
        </div>
    </div>
</div>
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    setInterval(loadTennisMatches, 60000);
});

function loadTennisMatches() {
    fetch('/api/cricket-matches')
        .then(response => response.json())
        .then(data => {
            const tennisMatches = data.matches.filter(m => m.sportType === 'tennis');
            displayMatches(tennisMatches);
        })
        .catch(error => {
            console.error('Error loading tennis matches:', error);
        });
}

function displayMatches(matches) {
    const container = document.getElementById('tennis-matches-container');
    
    if (matches.length === 0) {
        container.innerHTML = '<div class="alert alert-info">No tennis matches available at the moment.</div>';
        return;
    }
